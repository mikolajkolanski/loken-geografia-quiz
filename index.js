...
function start() {
    ...
    timer = setInterval(function() {
        ...
        timer_elem.innerText = Math.floor(delta/60000) + ':' + (Math.floor(delta/1000)%60 + '').padStart(2,'0') + '.' + (Math.floor(delta/10)%100 + '').padStart(2, '0');
    }, 10);
...
function end() {
    // To end this shit
    header.innerText = 'Wynik: ' + Math.round(score/(start_places.length*3)*100) + '%';
    clearInterval(timer)
    timer = null;
}
